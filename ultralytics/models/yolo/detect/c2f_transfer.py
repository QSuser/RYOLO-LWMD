...
def replace_c2f_v2_with_c2f(module):
    ...
    for name, child_module in module.named_children():
        if isinstance(child_module, C2f_CloAtt_v2):
            # Replace C2f with C2f_v2 while preserving its parameters
            shortcut = infer_shortcut(child_module.m[0])
            c2f = C2f_CloAtt_infer(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
...
def replace_c2f_with_c2f_v2(module):
    # for yolov8n.yaml
    for name, child_module in module.named_children():
        if isinstance(child_module, C2f_CloAtt):
            shortcut = infer_shortcut(child_module.m[0])
            c2f_v2 = C2f_CloAtt_v2(child_module.cv1.conv.in_channels, child_module.cv2.conv.out_channels,
                            n=len(child_module.m), shortcut=shortcut,
                            g=child_module.m[0].cv2.conv.groups,
                            e=child_module.c / child_module.cv2.conv.out_channels)
